...
class Attack:
    ...
    def __init__(self) -> None:
        ...
        self.letters_frequency_eng = {'a': 0.0817, 'b': 0.0150, 'c': 0.0278, 'd': 0.0425, 'e': 0.1270, 'f': 0.0223,
    ...
    def calculate_frequency(self, cipher):
        for char in self.alphabet:
            self.letters_frequency[char] = 0
        
        for char in cipher:
            if char in self.alphabet:
                self.letters_frequency[char] += 1
                self.letters_count += 1
        
        for char in self.letters_frequency:
            self.letters_frequency[char] = round(self.letters_frequency[char] / self.letters_count, 4)
    ...
    def print_frequency(self):
        ...
        for char in self.letters_frequency:
            lines_count += 1
            print(char, ':', self.letters_frequency[char], ' ', end='')
            if lines_count % 3 == 0:
                print()
    ...
    def calculate_matches(self):
        for cipher_char in self.alphabet:
            ...
            for plain_char in self.alphabet:
                map[plain_char] = round(abs(self.letters_frequency[cipher_char] - self.letters_frequency_eng[plain_char]), 4)
...
attack = Attack()
attack.calculate_frequency(cipher)
...
guess = attack.decrypt(key, cipher)
guess_lines = guess.splitlines()
cipher_lines = cipher.splitlines()
for i in range(len(guess_lines)):
    print('C:', cipher_lines[i])
    print('G:', guess_lines[i])